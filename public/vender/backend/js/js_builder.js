...
function list_body_switch() {
    $("body").on("click", ".btn_ctable", function (el) {
        ...
        if ($(this).parents('.for_ajax_content').find('.for_ajax_box .box').length < 1) {
            $(this).parents('.for_ajax_content').find(".list_headBar").addClass('top')
        }
        const box = _this.closest(".list").find(".list_frame");
        if (box.length > 0) {
            article_wysiwyg($(this).closest('.list'));
        ...
        // 特定段落樣式隱藏預設值
        if(_this.closest(".list").find("[name*='article_style']").val() == 'typeQuote'){
            _this.closest(".list").find("[name*='h_heading_tag_num']").closest('.inventory').hide();
            // _this.closest(".list").find("[name*='article_inner']").closest('.inventory').hide();
            _this.closest(".list").find("[name*='subh_align']").closest('.inventory').hide();
            _this.closest(".list").find("[name*='subh_color']").closest('.inventory').hide();
            _this.closest(".list").find("[name*='instagram_content']").closest('.inventory').hide();
            // _this.closest(".list").find("[name*='p_align']").closest('.inventory').hide();
            // _this.closest(".list").find("[name*='p_color']").closest('.inventory').hide();
        }
        else{
            _this.closest(".list").find("[name*='h_heading_tag_num']").closest('.inventory').show();
            // _this.closest(".list").find("[name*='article_inner']").closest('.inventory').show();
            _this.closest(".list").find("[name*='subh_align']").closest('.inventory').show();
            _this.closest(".list").find("[name*='subh_color']").closest('.inventory').show();
            _this.closest(".list").find("[name*='instagram_content']").closest('.inventory').show();
            // _this.closest(".list").find("[name*='p_align']").closest('.inventory').show();
            // _this.closest(".list").find("[name*='p_color']").closest('.inventory').show();
        }
        _this.closest(".list").find('.bk_spreadsheet').closest('.inventory').hide();
        _this.closest(".list").find("[name*='data_table_markdown']").closest('.inventory').hide();
        if(_this.closest(".list").find("[name*='article_style']").val() == 'typeTable'){
            _this.closest(".list").find("[name*='data_table_markdown']").closest('.inventory').show();
            _this.closest(".list").find('.bk_spreadsheet').closest('.inventory').show();
            _this.closest(".list").find("[name*='instagram_content']").closest('.inventory').hide();
        }
    });
}
...
function list_headBar_switch() {
    ...
    $("body").on("click", object2, function () {
        ...
        if (barID == 2 || barID == 3 || barID == 4 || barID == 5) {
            let article_type = $(this)
                .closest(".list_frame")
                .find("[name*='article_style']")
                .val();
            if(article_type=='typeTable'){
                alert("段落樣式請選擇非表格模式");
                return false;
            }
            if(article_type=='typeQuote'){
                alert("段落樣式請選擇非引言模式");
                return false;
            }
        }
...
function filePathSelect() {
    ...
    listObj.off("click").on("click", function (e) {
        ...
        pathText = pathTextArray.join(" / ");
        btnText.text($this.attr('data-path'));
        btnText.attr("title", pathText);
        listTop.slideUp(s, function() {
            $(this).removeClass('active')
            btnObj.removeClass('active')
