...
check.toObject = (object) => !!object.length;
check.toCallback = (callback) => typeof callback === "function" ? callback() : false;
...
uis.sidebar = (object) => {
    ...
    object.on("click", "li:not(.fms_unlimited_list) > a", function () {
        const levellist = $(this).parent();
        const levellist_All = $(this).parent().parent().find("li");
        const submenu = $(this).next("ul");
        const submenu_LV1 = $(this).parent().find("ul");
        const submenu_LV2 = $(this).parent().parent().find("ul");
        const submenu_All = object.find("li ul");
...
// uis.detailEditor_fixedHeader = () => {
//     let scrollAfter = 0;
//     let scrollBefore = 0;

//     $(".editorBody").on("scroll", function (e) {
//         scrollBefore = $(this).scrollTop();
//         const cover_H = $(this).height();
//         const object_H = $(this).find(".editorContent").height();
//         const scrollHeight = object_H - scrollBefore + 150;
...
uis.init = () => {
    ...
    setTimeout(() => {
        uis.sidebar($(".body-list .body-list"));
    }, 0);
    uis.mainnav($(".mainNav"));
    // uis.ceditorNav_sideBar();
    uis.switchMenu();
...
const datatableV2 = {}
datatableV2.dataTable = (object, callback) => {
    if (!check.toObject(object)) {
        return;
    }
